...
function parseICS(raw: string): ParsedEvent[] {
  ...
  for (const line of lines) {
    ...
    if (baseKey === 'BEGIN' && val.trim() === 'VEVENT') {
      current = { type: 'VEVENT' };
      continue;
    }
    if (baseKey === 'END' && val.trim() === 'VEVENT') {
    ...
    switch (baseKey) {
      case 'UID':
        current.uid = unescapeIcal(val.trim());
        break;
      case 'DTSTART':
        current.start = parseICalDate(line) ?? undefined;
        break;
      case 'DTEND':
        current.end = parseICalDate(line) ?? undefined;
        break;
      case 'SUMMARY':
        current.summary = unescapeIcal(val);
        break;
      case 'DESCRIPTION':
        current.description = unescapeIcal(val);
        break;
      case 'LOCATION':
        current.location = unescapeIcal(val);
        break;
    }
  }
  return events;
...
export class ICalExportService {
  ...
  async fetchReservations(
    ...
    if (request.daysAhead && request.daysAhead > 0) {
      ...
      cutoff.setDate(cutoff.getDate() + request.daysAhead);
