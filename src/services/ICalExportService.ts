...
export class ICalExportService {
  ...
  private parseICalData(icalData: string, sourceUrl: string, propertyName?: string): ICalReservation[] {
    ...
    while ((match = eventRegex.exec(icalData)) !== null) {
      ...
      if (summaryMatch && startMatch) {
        ...
        const end = endMatch ? this.parseICalDate(endMatch[1]) : new Date(start.getTime() + 3600000);
        let cleanSummary = summaryMatch[1].replace(/\\n/g, ' ').replace(/\\,/g, ',');
        if (cleanSummary.includes('CLOSED - Not available') || cleanSummary.includes('Not available')) cleanSummary = 'NIEDOSTĘPNE';
        else if (cleanSummary.includes('Reserved')) cleanSummary = 'ZAREZERWOWANE';
        else if (cleanSummary.includes('Airbnb (Not available)')) cleanSummary = 'NIEDOSTĘPNE (Airbnb)';
        reservations.push({
  ...
  private filterReservationsByDaysAhead(reservations: ICalReservation[], daysAhead: number): ICalReservation[] {
  ...
  private sortReservations(reservations: ICalReservation[], sortBy: 'start' | 'end' = 'start'): ICalReservation[] {
    return [...reservations].sort((a, b) => (sortBy === 'start' ? a.start.getTime() - b.start.getTime() : a.end.getTime() - b.end.getTime()));
  }
  ...
  async fetchReservations(request: ICalExportRequest): Promise<{ reservations: ICalReservation[]; summary: any }>{
    ...
    for (const url of urls) {
      try {
        ...
        all.push(...res);
        summary.successfulUrls++; summary.totalReservations += res.length;
      } catch (e) { summary.failedUrls++; summary.errors.push(`Błąd dla ${url}: ${e}`); }
    }
    let filtered = all;
